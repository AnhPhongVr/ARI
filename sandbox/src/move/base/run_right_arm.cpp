...
void waypoints_arm_goal(control_msgs::FollowJointTrajectoryGoal& goal)
{
  ...
  index += 1;
  goal.trajectory.points[index].positions.resize(4);
  goal.trajectory.points[index].positions[0] = 0.13;
  goal.trajectory.points[index].positions[1] = 1.70;
  goal.trajectory.points[index].positions[2] = 2.00;
  goal.trajectory.points[index].positions[3] = 1.90;
  // Velocities
  goal.trajectory.points[index].velocities.resize(4);
  for (int j = 0; j < 4; ++j)
  {
